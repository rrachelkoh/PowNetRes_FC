def DataSetup(run_no,dat_name):
    ...
    df_gen = pd.read_csv('../Data/data_camb_genparams.csv',header=0)
    ...
    df_hydro_import = pd.read_csv('../Data/data_camb_hydro_import_2016.csv',header=0)
    df_hydro_import = pd.concat([df_hydro_import] * num_yrs, ignore_index=True)
    
    ##hourly ts of dispatchable solar-power at each plant
    # df_solar = pd.read_csv('data_camb_solar.csv',header=0)   
    ...
    df_trans1 = pd.read_csv('../Data/data_camb_transparam.csv',header=0)
    ...
    h_nodes = ['KAMCHAY','ATAY','LRCHRUM','TATAY','KIRIROM1','KIRIROM3']
    h_imports = ['Salabam']
    # s_nodes = ['SOLAR1']
    ##w_nodes =['wind1','wind2']
    ...
    ####### simulation period and horizon
        ...
        f.write('\n\n')
        #f.write('param plant_cap := %0.3f;' % plant_cap)
        #f.write('\n\n')
    ...
    ####### cost of hydro import    
        f.write('param h_import_cost := %d;' % h_import_cost)
        f.write('\n\n')
