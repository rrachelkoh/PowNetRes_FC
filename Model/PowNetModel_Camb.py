...
model.hyd_periods = RangeSet(1,23)

#Info to run feedback loop
model.plant_cap = Param(model.h_nodes,within=NonNegativeReals,mutable=True)
...
model.HorizonHydro = Param(model.h_nodes,within=NonNegativeReals,mutable=True)
...
model.ini_mwh = Param(model.Generators, within=NonNegativeReals, mutable=True) #v1.3
...
model.hydro = Var(model.h_nodes,model.HH_periods,within=NonNegativeReals,initialize=0)
...
def SysCost(model):
    fixed = sum(model.maxcap[j]*model.fix_om[j]*model.on[j,i] \
                for i in model.hh_periods for j in model.Generators)
    starts = sum(model.maxcap[j]*model.st_cost[j]*model.switch[j,i] \
                 for i in model.hh_periods for j in model.Generators)
    ...
    slack = sum(model.mwh[j,i]*model.heat_rate[j]*model.gen_cost[j] \
                for i in model.hh_periods for j in model.Slack)
...
def HydroC2(model,j,i):
    if i==1:
        return model.hydro[j,i] <= model.HorizonHydro[j]
    else:    
        return model.hydro[j,i] <= model.HorizonHydro[j]-\
            sum(model.hydro[j,k] for k in model.hyd_periods if k<=i)
model.HydroConstraint2= Constraint(model.h_nodes,model.hh_periods,rule=HydroC2)

def HydroC3(model,j,i):
    return model.hydro[j,i] <= model.plant_cap[j]
model.HydroConstraint3= Constraint(model.h_nodes,model.hh_periods,rule=HydroC3)
...
def HPnodes_Balance(model,z,i):
    ...
    #demand = model.HorizonDemand[z,i]
    impedance=sum(model.linesus[z,k]*(model.vlt_angle[z,i]-model.vlt_angle[k,i]) \
                  for k in model.sinks)
...
def GN1_Balance(model,i):
    ...
    impedance = sum(model.linesus[gn_nodes[gn-1],k] * \
                    (model.vlt_angle[gn_nodes[gn-1],i] - model.vlt_angle[k,i]) \
                        for k in model.sinks)   
...
def SysReserve(model,i):
    return sum(model.srsv[j,i] for j in model.ResGenerators) + \
        sum(model.nrsv[j,i] for j in model.ResGenerators) >= model.HorizonReserves[i]
model.SystemReserve = Constraint(model.hh_periods,rule=SysReserve)

##Spinning Reserve Requirement
def SpinningReq(model,i):
    return sum(model.srsv[j,i] for j in model.ResGenerators) >= \
        model.spin_margin * model.HorizonReserves[i] 
